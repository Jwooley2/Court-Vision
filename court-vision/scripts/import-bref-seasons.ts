...
async function getTeamId(teamAbbreviation: string | undefined) {
  ...
  if (error) {
    throw new Error(`Error looking up team ${teamAbbreviation}: ${error.message}`);
  }
...
async function main() {
  ...
  for (const row of cleanRows) {
    ...
    const payload = {
      ...
      points_total: toInteger(
        getFirstValue(row, ["PTS_total", "Points", "PTS_TOT"])
      ),
      rebounds_total: toInteger(
        getFirstValue(row, ["TRB_total", "Rebounds", "TRB_TOT"])
      ),
      assists_total: toInteger(
        getFirstValue(row, ["AST_total", "Assists", "AST_TOT"])
      ),
      steals_total: toInteger(
        getFirstValue(row, ["STL_total", "Steals", "STL_TOT"])
      ),
      blocks_total: toInteger(
        getFirstValue(row, ["BLK_total", "Blocks", "BLK_TOT"])
      ),
      ...
      verification_notes:
        "Imported by Court Vision Basketball Reference CSV importer.",
    };
    ...
    console.log(`Imported ${seasonLabel} ${seasonType}`);
  }
